...
const useAssignmentStore = create<AssignmentState>((set) => ({
  ...
  editGrade: (updatedAssignment: {
    ...
    set((state) => {
      const updatedAssignments = Object.keys(state.assignment).reduce(
        (acc, date) => {
          const updatedDateAssignments = state.assignment[date]?.map(
            (assignment) => {
                ...
                const updatedStudents = assignment.students?.map((student) =>
                  student.student_id === updatedAssignment.student_id
                    ? {
                        ...
                        assignment_status: updatedAssignment.status,
                      }
                    : student
                );
