...
import { useRemoveClassroom } from '../../hooks/useClassroomQuery';
...
export const MoreModal: React.FC<({id: string})> = ({id}) => {
  ...
  return (
    <>
      ...
      <button onClick={() => handleMoreModal(id)}>
      ...
      {moreModalOpen === id && (
        <div className="w-[10rem] h-fit absolute right-2 top-[2rem] flex flex-col bg-white rounded-md p-2 shadow-lg border border-gray-200">
          ...
          {isEditModalOpen === id && (
            <EditModal onClose={() => closeEditModal()} id={id} onSubmit={handleEditModal}/>
          )}

          {/* Show delete modal for this assignment if it's open */}
          {isDeleteModalOpen === id && (
            <DeleteModal onClose={() => closeDeleteModal()} id={id} onSubmit={handleDeleteModal}/>
          )}
    </>
  );
};
