...
export const AssignmentBlock: React.FC<Props> = ({
  ...
  return (
    <>
      <div className="flex items-center gap-2 my-2 w-full cursor-pointer">
        <div className="flex flex-col bg-slate-200 p-3 rounded-md w-full border border-gray-300">
          <div className="flex items-start md:items-center justify-between">
            <div className="flex items-center gap-2">
              <Icon className="size-9 p-1.5 bg-white rounded-full border border-gray-300 text-primary" />
              <div className="flex flex-col items-start">
                <h1 className="font-bold text-left leading-4">{name}</h1>
              ...
              <div className="md:flex flex-col items-center hidden">
                <p className="text-sm">
                  {ass.assignment_status.toLowerCase() !== "returned" ? (
                    ...
                    <span className="font-bold ">{ass.grade}</span>
                  )}{" "}
                  / {ass.points}
                </p>
                <span className="md:block hidden text-xs opacity-60">
                  Your Grade
                </span>
              </div>
            )}
